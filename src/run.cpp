...
void setupRun(){
  ...
  deserializeJson(wifilist, file);
    ...
    off.attach(wifiOffTime, wifiOff);
		while (WiFi.status() != WL_CONNECTED) {delay(100); print(".");}
		off.detach();
		println(); println(WiFi.SSID() + " " + WiFi.RSSI() + " - " + millis());
	}
	else{
		isMulti = 1;
		if (WiFi.SSID() != ""){
...
void check(){
	if (isMulti){
		...
		if (voltage < 3100) { // 3100, доп. проверка, т.к. бывает ошибка
			// delay(500);
			voltage = ESP.getVcc();
			println(String("V-") + voltage);
			if (voltage < 3000) {
  	...
  	localtime_r(&now, &tm);
		...
		if (tm.tm_year != 70){
  		...
  		deserializeJson(schedule, file);
			...
			if (flag){ // && event != 10 // Если в расписании пусто + если не включён Anki через event
				println(String("Sleep ") + sleepTime + " s");
				ESP.deepSleep(sleepTime * 1e6 + 1e6);
			}
			else {
        #if defined(ESP32)
          ledcWrite(1, 1023); // digitalWrite(R_PIN, 0);

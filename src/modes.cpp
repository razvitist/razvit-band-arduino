...
void sleep(int e, int minutes)
{
  print("Sleep ");
  if (e - minutes <= 60)
  { // 200 изменено для работы пищалки// Есть ограничение на время сна
...
void anki()
{
  ...
  {
    setpin("anki", 0);
    print(String(min(config["ankiOne"].as<int>() * anki, config["ankiMax"].as<int>())));
    println(" s");
...
void github()
{
  ...
  {
    print(String(min((githubNew - githubOld) * config["githubOne"].as<int>(), config["githubMax"].as<int>())));
    println(" s");
...
void yandex()
{
  ...
  if (yandexNew - yandexOld > 0)
  {
    print(String(min((yandexNew - yandexOld) * config["yandexOne"].as<int>(), config["yandexMax"].as<int>())));
    println(" s");
    ESP.deepSleep(min((yandexNew - yandexOld) * config["yandexOne"].as<int>(), config["yandexMax"].as<int>()) * 1e6 + 1e6);
  }
...
void video()
{
  ...
  if (vNew - vOld > 0)
  {
    print(String(min(config["videoOne"].as<int>() * (vNew - vOld), config["videoMax"].as<int>())));
    println(" s");
...
{ // Такое же как video
  ...
  if (vNew - vOld > 0)
  {
    print(String(min(config["extensionOne"].as<int>() * (vNew - vOld), config["extensionMax"].as<int>())));
    println(" s");
...
{ // Такое же как video
  ...
  if (vNew - vOld > 0)
  {
    print(String(min(config["programOne"].as<int>() * (vNew - vOld), config["programMax"].as<int>())));
    println(" s");
...
void extensionOld(String name, String request, boolean isVideo, boolean isSec = false)
{
  if (isVideo)
  {
    ...
    if (VNew - VOld > 0)
    {
      print(String(min(config[name + "VOne"].as<int>() * (VNew - VOld), config[name + "VMax"].as<int>())));
      println(" s");
...
void custom(String name)
{
  ...
  if (vNew - vOld > 0)
  {
    print(String(min(one * (vNew - vOld), maximum)));
    println(" s");

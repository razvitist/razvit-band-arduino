...
// }
    ...
    server.on("/get-battery", HTTP_GET, [](AsyncWebServerRequest *request)
              { request->send(200, "text/plain", String(int(ESP.getVcc() / 40)).c_str()); });
#elif defined(ESP32)
    server.on("/get-battery", HTTP_GET, [](AsyncWebServerRequest *request)
              { request->send(200, "text/plain", String(constrain(map(analogRead(V_PIN), 3000.0f, 4000.0f, 0, 100), 1, 100)).c_str()); });
    server.on("/get-charge", HTTP_GET, [](AsyncWebServerRequest *request)
              {
                request->send(200, "text/plain", String(!digitalRead(STDBY_PIN) ? "Standby" : (!digitalRead(CHRG_PIN) ? "Charge" : "")).c_str());
                // request->send(200, "text/plain", (String(analogRead(V_PIN)) + " " + String(map(analogRead(V_PIN), 0.0f, 4095.0f, 0, 100)) + String(" STDBY ") + digitalRead(STDBY_PIN) + " " + analogRead(STDBY_PIN) + String(" CHRG ") + digitalRead(CHRG_PIN) + " " + analogRead(CHRG_PIN)).c_str());
              });
    const char *pages[] = {"about", "alarm", "anki", "battery", "codewars", "custom", "extension", "github", "import", "leetcode", "logs", "main", "program", "schedule", "stepik", "video", "wifi", "yandex"};
